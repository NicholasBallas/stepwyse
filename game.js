    ...
    try {
        ...
        if (!isRelated) {
            ...
            const override = confirm(
                `"${word.toUpperCase()}" doesn't seem directly related to "${getCurrentWord()}". ` +
                `Do you have a creative connection? Click OK to explain, or Cancel to try a different word.`
            );
...
async function validateSemanticConnection(word1, word2) {
    ...
    try {
        ...
        const results = await Promise.all(
            endpoints.map(url => fetch(url).then(r => r.json()).catch(() => []))
        );
